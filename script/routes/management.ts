...
const APP_VERSION_RANGE_REGEX = /[*~^<>=\s-]|\.x/;
const CACHE_PURGE_ENABLED = process.env.CACHE_PURGE_ENABLED === 'true';
          ...
          return storage
            ...
            .then((app: storageTypes.App): Promise<string[]> => {
              ...
              if (!appRequest.manuallyProvisionDeployments) {
                const defaultDeployments: string[] = ["Production"];
  ...
  router.patch("/apps/:appName/deployments/:deploymentName/release", (req: Request, res: Response, next: (err?: any) => void): any => {
    ...
    nameResolver
      ...
      .then((packageHistory: storageTypes.Package[]) => {
        ...
        if (CACHE_PURGE_ENABLED) {
          const oldAppVersion = packageToUpdate?.appVersion;
          const appDeploymentKey = storageDeployment?.key;
          console.log(`APP_DEPLOYMENT_KEY: ${appDeploymentKey}, APP_VERSION:${oldAppVersion}`)
        
          if (!(appDeploymentKey || oldAppVersion)) {
            console.warn('SKIPPING CLOUDFLARE PURGE: MISSING DEPLOYMENT KEY OR APP VERSION.');
          } else {
            const reqData = {
              deploymentKey: appDeploymentKey,
              appVersion: oldAppVersion,
              isRange: true,
            };
            const headers = {
              'x-server-secret': process.env.SERVER_SECRET_KEY || '',
            };
            const url = process.env.CLOUDFLARE_WORKER_URL;
        
            console.log('PURGE REQUEST DATA:', JSON.stringify(reqData));
            console.log('PURGE REQUEST HEADERS:', JSON.stringify(headers));
      
            axios
              .post(url, reqData, { headers })
              .then((response) => {
                if (response.status === 200) {
                  console.log('CLOUDFLARE PURGE RESPONSE:', response.data);
                  console.log(`CLOUDFLARE CACHE PURGED FOR DEPLOYMENT_KEY: ${appDeploymentKey}, VERSION: ${oldAppVersion}, RANGE: ${reqData.isRange}`);
                } else {
                  console.warn(`UNEXPECTED STATUS CODE FROM CLOUDFLARE PURGE: ${response.status}`);
                }
              })
              .catch((error) => {
    ...
    fs.stat(filePath, (err: NodeJS.ErrnoException, stats: fs.Stats): void => {
      ...
      nameResolver
        ...
        .then((history: storageTypes.Package[]) => {
          ...
          // CLOUDFLARE CACHE PURGE
          if (CACHE_PURGE_ENABLED) {
            let oldAppVersion: string;
            const appDeploymentKey = deploymentToReleaseTo?.key

            if (history && history.length) {
              const lastPackageIndex: number = history.length - 1;
              oldAppVersion = history[lastPackageIndex]?.appVersion;
            }
            console.log(`APP_DEPLOYMENT_KEY: ${appDeploymentKey}, APP_VERSION:${oldAppVersion}`)
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      nameResolver
        ...
        .then((packageHistory: storageTypes.Package[]) => {
          ...
          // CLOUDFLARE CACHE PURGE
          if (CACHE_PURGE_ENABLED) {
            const oldAppVersion = destinationPackage?.appVersion;
            const appDeploymentKey = deploymentToRollback?.key;
            console.log(`APP_DEPLOYMENT_KEY: ${appDeploymentKey}, APP_VERSION:${oldAppVersion}`)
