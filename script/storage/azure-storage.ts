...
export class AzureStorage implements storage.Storage {
  ...
  private _blobProvider: BlobStorageProvider;

  public constructor(accountName?: string, accountKey?: string) {
    shortid.characters("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-");
    console.log("📊 Data storage: Always using Azure Table Storage");
    console.log(`🗄️ File storage: Using ${process.env.STORAGE_TYPE || "AZURE"}`);
  ...
  public checkHealth(): q.Promise<void> {
    return this._setupPromise
      .then(() => {
        // Always check the table health since data is always in Azure Tables
        const tableCheck: q.Promise<void> = q.Promise<void>((resolve, reject) => {
          this._tableClient
            .getEntity("health", "health")
            .then((entity: any) => {
        ...
        // Check blob health but don't fail overall health check if blob fails
        const blobChecks = [
          this.blobHealthCheck(AzureStorage.TABLE_NAME),
          this.blobHealthCheck(AzureStorage.HISTORY_BLOB_CONTAINER_NAME)
        ];

        // Return success as long as table health succeeds, regardless of blob health
        return tableCheck.then(() => {
          // We'll check blob health in the background but not fail the health check
          q.allSettled(blobChecks).then(results => {
            for (const result of results) {
              if (result.state === "rejected") {
                console.warn("Blob health check failed:", result.reason);
              }
            }
          });
  ...
  public addBlob(blobId: string, stream: stream.Readable, streamLength: number): q.Promise<string> {
    return this._setupPromise
      ...
      .then((buffer) => {
        try {
          console.log(`Uploading blob ${blobId} to ${process.env.STORAGE_TYPE || 'AZURE'} storage, size: ${buffer.byteLength} bytes`);
          return this._blobProvider.uploadBlob(AzureStorage.TABLE_NAME, blobId, Buffer.from(buffer), buffer.byteLength);
        } catch (error) {
          console.error("Error in addBlob:", error);
          throw storage.storageError(storage.ErrorCode.Other, `Failed to upload blob: ${error.message || JSON.stringify(error)}`);
        }
      })
      .then(() => {
        console.log(`Successfully uploaded blob ${blobId}`);
        return blobId;
      })
      .catch((error) => {
        console.error("Error in addBlob outer catch:", error);
        return AzureStorage.azureErrorHandler(error);
      });
  }
  ...
  public getBlobUrl(blobId: string): q.Promise<string> {
    return this._setupPromise
      .then(() => {
        try {
          return this._blobProvider.getBlobUrl(AzureStorage.TABLE_NAME, blobId);
        } catch (error) {
          console.error("Error in getBlobUrl:", error);
          throw storage.storageError(storage.ErrorCode.Other, `Failed to get blob URL: ${error.message}`);
        }
      })
      .catch(AzureStorage.azureErrorHandler);
  }
  ...
  public removeBlob(blobId: string): q.Promise<void> {
    return this._setupPromise
      .then(() => {
        try {
          return this._blobProvider.deleteBlob(AzureStorage.TABLE_NAME, blobId);
        } catch (error) {
          console.error("Error in removeBlob:", error);
          throw storage.storageError(storage.ErrorCode.Other, `Failed to remove blob: ${error.message}`);
        }
      })
      .catch(AzureStorage.azureErrorHandler);
  }
  ...
  private setup(accountName?: string, accountKey?: string): q.Promise<void> {
    ...
    try {
      ...
      // IMPORTANT: Always initialize Azure Table Storage regardless of STORAGE_TYPE
      ...
      // Create the blob provider based on STORAGE_TYPE
      this._blobProvider = StorageFactory.createBlobStorageProvider();
      
      const tableHealthEntity: any = this.wrap({ health: "health" }, /*partitionKey=*/ "health", /*rowKey=*/ "health");

      // Using q.ninvoke or a wrapper function to convert from Promise to Q Promise
      // For TableServiceClient.createTable
      const createTablePromise = q.Promise<void>((resolve, reject) => {
      ...
      createTablePromise
        .then(() => {
          // For TableClient.createEntity - here's the fix:
          return q.Promise<void>((resolve, reject) => {
            tableClient.createEntity(tableHealthEntity)
              .then(() => resolve()) // Ignore the return value and just resolve with void
              .catch(reject);
          });
        })
        .then(() => {
          // For blob container creation
          return q.all([
            q.Promise<void>((resolve, reject) => {
              this._blobProvider.createContainer(AzureStorage.TABLE_NAME, { access: "blob" })
                .then(() => resolve())
                .catch(reject);
            }),
            q.Promise<void>((resolve, reject) => {
              this._blobProvider.createContainer(AzureStorage.HISTORY_BLOB_CONTAINER_NAME)
                .then(() => resolve())
                .catch(reject);
            })
          ]);
        ...
        .then(() => {
          ...
          return q.all([
            q.Promise<void>((resolve, reject) => {
              this._blobProvider.uploadBlob(AzureStorage.TABLE_NAME, "health", "health", "health".length)
                .then(() => resolve())
                .catch(reject);
            }),
            q.Promise<void>((resolve, reject) => {
              this._blobProvider.uploadBlob(AzureStorage.HISTORY_BLOB_CONTAINER_NAME, "health", "health", "health".length)
                .then(() => resolve())
                .catch(reject);
            })
          ]);
        ...
        .then(() => {
          // Set the table client
          this._tableClient = tableClient;
          deferred.resolve();
        })
        .catch((error) => {
          if (error.code === "TableAlreadyExists" || error.code === "EntityAlreadyExists" || error.code === "ContainerAlreadyExists") {
            // Ignore expected errors during initialization
            this._tableClient = tableClient;
            deferred.resolve();
          } else {
            console.warn("Error during setup:", error);
            // Even if blob operations fail, we should still initialize the table client
            if (!this._tableClient && tableClient) {
              this._tableClient = tableClient;
            }
            // Only throw errors that prevent core functionality
            if (!this._tableClient) {
              deferred.reject(error);
            } else {
              // If table client is set, we can continue despite blob errors
              deferred.resolve();
            }
          }
        });
  ...
  private blobHealthCheck(container: string): q.Promise<void> {
    ...
    this._blobProvider.checkHealth(container)
      .then(() => {
        deferred.resolve();
      })
      .catch((error: any) => {
  ...
  private getPackageHistoryFromBlob(blobId: string): q.Promise<storage.Package[]> {
    ...
    this._blobProvider.downloadBlob(AzureStorage.HISTORY_BLOB_CONTAINER_NAME, blobId)
  ...
  private uploadToHistoryBlob(blobId: string, content: string): q.Promise<void> {
    ...
    this._blobProvider.uploadBlob(AzureStorage.HISTORY_BLOB_CONTAINER_NAME, blobId, content, content.length)
      .then(() => {
        deferred.resolve();
      })
      .catch((error: any) => {
  ...
  private deleteHistoryBlob(blobId: string): q.Promise<void> {
    ...
    this._blobProvider.deleteBlob(AzureStorage.HISTORY_BLOB_CONTAINER_NAME, blobId)
      .then(() => {
        deferred.resolve();
      })
      .catch((error: any) => {
