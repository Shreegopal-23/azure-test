  ...
  constructor() {
    if (process.env.REDIS_HOST && process.env.REDIS_PORT) {
      const redisConfig = {
        ...
        password: process.env.REDIS_KEY, // Set the password directly to match Docker Compose
        legacyMode: false
      };
      ...
      this._opsClient.on("connect", () => {
        console.log("Redis Ops Client Connected");
      });
    
      this._metricsClient.on("connect", () => {
        console.log("Redis Metrics Client Connected");
      });
    
      // Connect the ops client
      this._opsClient.connect().catch(err => {
        console.error("Failed to connect ops client:", err);
      });
